...
//        member.setMembertags(membertags);
    ...
    public RestResponse getInfo(@RequestParam("mobile") String mobile) throws IOException {
        ...
        if (list.size() > 0) {
            ...
            deptlist = qkjvipMemberDatadepService.queryAll(params);
            userlist = qkjvipMemberOrguserService.queryAll(params);
            // 查询该会员已发放的礼包
            params.clear();
            params.put("memberid", member.getMemberId());
...
//            }
        ...
        String resultPost = HttpClient.sendPost(Vars.MEMBER_ADDPERM_URL, queryJsonStr);
        //插入会员标签
        JSONObject resultObject = JSON.parseObject(resultPost);
        if ("200".equals(resultObject.get("resultcode").toString())) {  //调用成功
    ...
    public RestResponse save(@RequestBody QkjvipMemberImportEntity memberImport) {
        memberImport.setMemberId("");  // 将id设置为空，以防主键重复
        ...
        try {
            ...
            if ("200".equals(resultObject.get("resultcode").toString())) {  //清洗成功
                ...
                // check该会员是否已发放了礼包福利
                // 用户设置了福利包
                if (!StringUtils.isEmpty(memberImport.getWelfareid())) {
                    params.clear();
                    params.put("memberid", member.getMemberId());
                    params.put("scenetype", 2);
                    List<QkjrtsWelfareSendrecordEntity> sendrecordList = qkjrtsWelfareSendrecordService.queryAll(params);
                    if (sendrecordList.size() == 0) {
                        ...
                        sendrecord.setWelfareid(memberImport.getWelfareid());
                        sendrecord.setWelfarename(memberImport.getWelfarename());
                        sendrecord.setScenetype(2);
                        sendrecord.setWelfaretype(8);
                        if (memberImport.getPeriodtype() != null && memberImport.getPeriodtype() == 1) { // 相对日期
                            Date startDate = DateUtils.setDate(new Date());
                            sendrecord.setStartvaliddate(startDate);
                            sendrecord.setEndvaliddate(DateUtils.addDateDays(startDate, memberImport.getPerioddays()));
                        } else if (memberImport.getPeriodtype() != null && memberImport.getPeriodtype() == 0) {  // 绝对日期
                            sendrecord.setStartvaliddate(memberImport.getStartvaliddate());
                            sendrecord.setEndvaliddate(memberImport.getEndvaliddate());
                        }
                        qkjrtsWelfareSendrecordService.add(sendrecord);
                    }
                }
                // 更新地址
//                memberImport.setMemberId(resultObject.get("memberid").toString());
//                setMemberAddress(JSON.toJSONString(memberImport));
...
//                setMemberAddress(memberJsonStr);
            }
        } catch (IOException e) {
            e.printStackTrace();
        }
    ...
    public void setMemberAddress(String memberJson) {
        MemberEntity member = JSON.parseObject(memberJson, MemberEntity.class);
        if (member.getMembershiptype() != null && member.getMembershiptype() == 1) {
            memberAddressService.deleteByMemberId(member.getMemberId());
            MemberAddressEntity maddress = new MemberAddressEntity();
            maddress.setMemberId(member.getMemberId());
            maddress.setProvince(member.getProvince());
            maddress.setCity(member.getCity());
            maddress.setArea(member.getDistrict());
            maddress.setStreet(member.getAddress());
            maddress.setConName(member.getRealName());
            maddress.setMobile(member.getMobile());
            maddress.setDefaultAddress(1);
            maddress.setAddUser(getUserId());
            maddress.setAddDept(getOrgNo());
            maddress.setAddTime(new Date());
            memberAddressService.add(maddress, null);
        }
    }
...
//                            return RestResponse.error("第" + rownum + "行收货地址为空，请修改后重新上传！");
//                        }
     ...
     * @return RestResponse
     */
    @GetMapping("/otherTabUserListInfo/{memberId}")
    public RestResponse otherTabUserListInfo(@PathVariable("memberId") String memberId) throws IOException {
        MemberEntity member = new MemberEntity();
        //获取会员标签
        Map<String, Object> params = new HashMap<>();
        params.put("memberId", memberId);
