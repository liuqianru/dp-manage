...
public class JedisUtil {
    ...
    // 批量执行添加缓存操作
    public void setpipelineObject(Map<String, Object> map,String keys, String title){
        long pipelineStart = System.currentTimeMillis();
        Jedis jedis = null;
        try {
            jedis = getResource();
            Pipeline pipeline = jedis.pipelined();
            pipeline.set(keys,title); // 键个数
            for(Map.Entry<String, Object> entry:map.entrySet()){
                pipeline.set(getBytesKey(entry.getKey()),toBytes(entry.getValue()));
            }
            pipeline.sync();
        } catch (Exception ignored) {
        } finally {
        ...
        System.out.println("pipeline操作Objeck数据类型set写入，耗时：" + (pipelineEnd - pipelineStart) + "毫秒");
